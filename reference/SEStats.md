...
- **Cutoffs** are derived from column names in each `data.frame` of
  `stats_dfs` (columns beginning with "hit ")
...
- `sedesign`:
  ...
  `SEDesign` object containing the experimental design and contrast
  information.

- `stats_dfs`:

  nested `list` structure organizing `data.frame` of statistical
  results. The structure is `stats_dfs[[signal]][[contrast]]` where each
  `data.frame` contains detailed statistical metrics for a specific
  signal type (assay) and contrast comparison.
