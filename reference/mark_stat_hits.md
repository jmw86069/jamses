...
- assign_value:
  ...
  `"logfc"` uses the log2 fold change value. If there is no matching
  colname for `logfc_colname` then all hits are assigned `1`. In all
  cases, entries which are not hits are assigned `0`.
