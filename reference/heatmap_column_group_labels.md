...
that used the argument `column_split` to sub-divide the heatmap columns
into subgroups. This function draws labels above the heatmap to describe
...
- The arguments `hm_title_base` and `hm_body_base` should match the
  heatmap name, which is defined in
  [`heatmap_se()`](https://jmw86069.github.io/jamses/reference/heatmap_se.md)
  with `data_type`, usually prefixed `"centered\n"` when the data is
  centered by that function. For example, when `data_type="abundance"`,
  the corresponding argument value should be
...
- use
  ...
  to help identify the appropriate `grid` elements available to be used
  by this function.
...
argument `hm_title_buffer` in
[`heatmap_se()`](https://jmw86069.github.io/jamses/reference/heatmap_se.md)).
However, when element `"global_column_title"` is present, the
`y_offset_lines` are not adjusted, so that the position is above the
column_title region. In this case, to position the labels below the
column_title, you can adjust `y_offset_lines` down manually like this:
`y_offset_lines=-9`.
