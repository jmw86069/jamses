...
unable to fill this purpose, since it is not available for
[`quartz()`](https://rdrr.io/r/grDevices/quartz.html) and `windows()`
devices, and only works properly with a subset of graphics devices such
as PDF, SVG, PNG, etc. That said, it would otherwise be the preferred
