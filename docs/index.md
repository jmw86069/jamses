...
- A two-way contrast is defined as the fold change of two
  ***compatible*** one-way fold changes, and in the proper order.  
...
- [`groups_to_sedesign()`](reference/groups_to_sedesign.md) takes by
  default a `data.frame` where each column represents an experiment
  factor, and creates the following:
...
- **`samples`**: a `character` vector of sample names, derived from
  [`colnames()`](https://rdrr.io/r/base/colnames.html) of the
...
- **`design`**: a `matrix` representing the design matrix, with
  ...
  - [`rownames()`](https://rdrr.io/r/base/colnames.html) are always
    maintained, so they can be aligned with
    [`colnames()`](https://rdrr.io/r/base/colnames.html) of the
    `SummarizedExperiment` object. This requirement helps ensure that
    the design and assay data are always in proper sync.
  - [`colnames()`](https://rdrr.io/r/base/colnames.html) are defined as
    sample groups, equivalent to using `model.matrix( ~ 0 + groups )`
    for example from the [The Limma User’s
    Guide](https://www.bioconductor.org/packages/devel/bioc/vignettes/limma/inst/doc/)
...
- [`se_normalize()`](reference/se_normalize.md) - lightweight wrapper to
  several normalization functions:

  - `method="jammanorm"`: calls
    [`jamma::jammanorm()`](https://rdrr.io/pkg/jamma/man/jammanorm.html)
    which normalizes the median log fold change to zero. This method is
    also used in `DESeq2::estimateSizeFactors()` for example. When using
    [`jamma::jammaplot()`](https://rdrr.io/pkg/jamma/man/jammaplot.html)
    for MA-plots, the `jammanorm()` method is conceptually equivalent to
    shifting the y-axis values to zero, so the median log fold change is
    zero. (Assumptions apply.)
  - `method="quantile`: calls
  ...
  - `method="limma_batch_adjust"` or `method="lba"`: calls
    ...
    to adjust of batch effects. This normalization is only recommended
    for visualization and clustering, not recommended prior to
    statistical comparisons. Instead, the recommendation is to define a
    blocking factor passed to
    [`se_contrast_stats()`](reference/se_contrast_stats.md) with
    argument `block`.

- [`matrix_normalize()`](reference/matrix_normalize.md) - is the core
  function for [`se_normalize()`](reference/se_normalize.md) and
...
- Options:
  ...
  - `block`: optional blocking factor, which enables the
    ...
    calculation, which is applied to the model fit per the [The Limma
    User’s
    Guide](https://www.bioconductor.org/packages/devel/bioc/vignettes/limma/inst/doc/).
...
- Save to excel with [`save_sestats()`](reference/save_sestats.md):
...
[`heatmap_se()`](reference/heatmap_se.md) is a wrapper for the amazing
...
- `sestats`: optional stat hits in one of these formats:
  ...
  - output from [`se_contrast_stats()`](reference/se_contrast_stats.md)
