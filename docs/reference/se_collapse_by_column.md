...
- `colData(se)` will also be collapsed by
  [`shrinkDataFrame()`](shrinkDataFrame.md) to combine unique values
  from each column annotation.
...
[`se_collapse_by_row()`](se_collapse_by_row.md), in this case the
function is mostly convenient method to calculate group mean values in
context of a `SummarizedExperiment` object, so it can be used with
[`jamses::heatmap_se()`](heatmap_se.md) for example.
