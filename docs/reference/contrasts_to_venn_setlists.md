...
- sestats:
  ...
  [`se_contrast_stats()`](se_contrast_stats.md), used only when
...
- contrast_style:
  ...
  - `"factors"` returns a `data.frame` from
    [`contrasts_to_factors()`](contrasts_to_factors.md) with one column
    per design factor. In this case it may be useful to pass
    `factor_names` in order to assign column names to the factor
    columns.
