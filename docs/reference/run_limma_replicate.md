...
- "stats_df": `list` of individual `data.frame` per contrast, each
  result is the output from [`ebayes2dfs()`](ebayes2dfs.md).
...
This function is called by [`se_contrast_stats()`](se_contrast_stats.md)
to perform the comparisons defined as contrasts. The
[`se_contrast_stats()`](se_contrast_stats.md) function operates on a
`SummarizedExperiment` object, this function operates on the `numeric`
`matrix` values directly.

This function also calls [`ebayes2dfs()`](ebayes2dfs.md) which extracts
each contrast result as a `data.frame`, whose column names are modified
to include the contrast names.
