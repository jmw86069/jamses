...
- sedesign:
  ...
  [`groups_to_sedesign()`](groups_to_sedesign.md), with slotNames
  `"design"`, `"contrasts"`, and `"samples"`. The arguments `idesign`
  and `icontrasts` are ignored when this argument is defined.
...
- handle_na:
  ...
  matrix, passed to [`handle_na_values()`](handle_na_values.md).
...
- na_value:
  ...
  `numeric` passed to [`handle_na_values()`](handle_na_values.md)
...
[`jamses::groups_to_sedesign()`](groups_to_sedesign.md). That function
assigns each sample to a sample group, then assembles all relevant group
contrasts which involve only one-factor contrast at a time. It
optionally defines two-factor contrasts (contrast of contrasts) where
applicable.
...
new_fold <- rnorm(20);
se2 <- cbind(se, se)
#> Error: unable to find an inherited method for function 'bindCOLS' for signature 'x = "SummarizedExperiment"'
...
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'colData': object 'se2' not found
...
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'assays': object 'se2' not found
...
#> Error: object 'sedesign2' not found
...
sestats2 <- se_contrast_stats(se=se2,
   sedesign=sedesign2,
   assay_names="counts")
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'colnames': object 'se2' not found

# print data.frame summary of hits
sestats_to_df(sestats2)
#> Error: object 'sestats2' not found
...
#> Error: object 'sestats2' not found
jamba::sdim(hit_list);
#> Error: object 'hit_list' not found
