...
- sestats:
  ...
  - `list` object output from
    [`se_contrast_stats()`](se_contrast_stats.md), containing
    `"hit_array"`
  ...
  - `list` of `character` vectors representing `rownames(se)` for the
    parent [`heatmap_se()`](heatmap_se.md) function.
...
- cutoff_names, contrast_names, assay_names:
  ...
  [`hit_array_to_list()`](hit_array_to_list.md) when the input is
  `sestats` or `hit_array`.
...
- ...:
  ...
  [`contrast2comp()`](contrast2comp.md) if relevant.
