...
- contrast_pattern:
  ...
  - The pattern is essentially any non-whitespace string that contains a
    ...
    assumed to be a comp (see [`comp2contrast()`](contrast2comp.md))
...
`list` consistent with [`se_contrast_stats()`](se_contrast_stats.md)
output, except limited to elements: 'stats_df', 'hit_array', 'hit_list'.
The content should be sufficient for use in
[`heatmap_se()`](heatmap_se.md) and other related functions.

## Details

This function converts a list of stat `data.frame` objects to a `list`
object as returned by [`se_contrast_stats()`](se_contrast_stats.md). In
future this function will return a S4 `SEStats` object.
